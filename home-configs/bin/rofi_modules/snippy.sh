...
CLIPASTE="xdotool key ctrl+alt+v"
...
run(){
  ...
  FILE=`find -L .  -type f | grep -v '^\.$' | sed 's!\.\/!!' | /usr/bin/rofi -dmenu ${DMENU_ARGS}`
  if [ -f ${DIR}/${FILE} ]; then
  ...
  paste_cli || paste_gui # cli or gui paste
}
...
paste_cli(){
  ...
  [[ ! "$name" =~ urxvt|term|tilda ]] && return 1
  xsel -b --input < $TMPFILE
  ${CLIPASTE}
}
